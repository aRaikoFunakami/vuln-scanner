...
def test_symlink_escape_and_size_guard():
    ...
    # a fifo / device named like a manifest must not hang the read (its
    # size reads as 0, slipping a naive size-only guard) -- reject
    # non-regular files (issue #28 review).
    if hasattr(os, "mkfifo"):
        with tempfile.TemporaryDirectory() as root:
            os.mkfifo(os.path.join(root, "requirements.txt"))
            findings, _n, _i = scan_local(root, None)  # must return, not block
        assert any(f["verdict"] == NOT_ANALYZED for f in findings), findings

    # a real large lockfile (below the cap) must still be parsed -- the
    # cap is well above real monorepo lockfile sizes so transitive-only
    # vulnerabilities are not silently missed (issue #28 review).
    with tempfile.TemporaryDirectory() as root:
        with open(os.path.join(root, "package.json"), "w") as f:
        ...
        with open(os.path.join(root, "package-lock.json"), "w") as f:
            json.dump(
                {"packages": {"node_modules/axios": {"version": "1.14.1"}},
                 "_pad": "x" * (6 * 1024 * 1024)}, f,
            )
        findings, _n, _i = scan_local(root, None)
    assert any(
        f["package"] == "axios" and f["verdict"] == VULNERABLE for f in findings
    ), "transitive vuln in a 6MB lockfile must still be detected"
